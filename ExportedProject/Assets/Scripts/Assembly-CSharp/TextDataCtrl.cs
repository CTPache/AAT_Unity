...
public static class TextDataCtrl
{
	...
	public static void SetLanguage(Language in_language)
	{
		Debug.Log(string.Concat("SetLanguage in_language:", in_language, " current_language_", current_language_));
		if (in_language != (Language)current_language_)
		{
			LoadTextData(ref common_text_data_, common_text_path_[(int)in_language], in_language);
			LoadTextData(ref title_text_data_, title_text_path_[(int)in_language], in_language);
			LoadTextData(ref option_text_data_, option_text_path_[(int)in_language], in_language);
			LoadTextData(ref save_text_data_, save_text_path_[(int)in_language], in_language);
			LoadTextData(ref platform_text_data_, platform_text_path_[(int)in_language], in_language);
			LoadTextData(ref system_text_data_, system_text_path_[(int)in_language], in_language);
			current_language_ = (int)in_language;
		}
	}
	...
	private static void LoadTextData(ref ConvertTextData data, string path, Language language)
	{
	...
	private static void LoadTextData(ref ConvertLineData data, string path, Language language)
	{
		string in_path = Application.streamingAssetsPath + path;
		byte[] bytes = decryptionCtrl.instance.load(in_path);
