...
public class optionSelectItem : optionItem
{
    ...
    public override void Init()
    {
        if (!isInit)
        {
            for (int i = 0; i < Language.languages.Count - 7; i++)
                setUpGuideSprite();
            isInit = true;
        }
        base.Init();
    ...
    private void setUpGuideSprite()
    {
        if (this.select_.select_guide_.Count > 0)
        {
            int size = this.select_.select_guide_.Count - 1;
            SpriteRenderer spriteRenderer = new GameObject
            {
                transform =
                {
                    parent = this.select_.select_guide_[size].sprite_renderer_.transform.parent,
                    localScale = this.select_.select_guide_[size].sprite_renderer_.transform.localScale,
                    localPosition = this.select_.select_guide_[size].sprite_renderer_.transform.localPosition,
                    position = this.select_.select_guide_[size].sprite_renderer_.transform.position
                },
                name = "option_button",
            ...
            assetBundleSprite.load("/menu/option/", "option_button", false);
            this.select_.select_guide_.Add(assetBundleSprite);
        }
    }
