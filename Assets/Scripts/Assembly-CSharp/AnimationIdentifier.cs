...
public class AnimationIdentifier : MonoBehaviour
{
    ...
    public string AnimationNameToBundlePath(string animation_name)
    {
        ...
        if (Language.languages.IndexOf(GSStatic.global_work_.language) > 6)
            path += "/../LangPacks/" + GSStatic.global_work_.language;
        //Debug.Log(path);
        int num = 0;
        switch (CurrentTitleId)
        {
        ...
        for (int i = num; i < dictionary_instances.Count; i++)
        {
            ...
            foreach (AnimationDictionary.TextureAnmNameRelation relation in animationDictionary.Relations)
            {
                if (relation.Values.Contains(animation_name))
                {
                    ...
                    if (File.Exists(path + string.Format(text + "/TextureAnmRelations/{0}/{1}.unity3d", animationDictionary.name, relation.Key)))
                        return path + string.Format(text + "/TextureAnmRelations/{0}/{1}.unity3d", animationDictionary.name, relation.Key);
                    else
    ...
    public void LoadDictionary(string path)
    {
        if (!isInitializingStarted)
        {
            ...
            coroutineCtrl.instance.Play(LoadDictionaryFlow(path));
            replacement_instances = new List<FoaReplacement>();
            coroutineCtrl.instance.Play(LoadReplacementDictionaryFlow(path));
        }
    }

    private IEnumerator LoadDictionaryFlow(string path)
    {
        //Debug.Log(path);

        List<string> gs1Dictionaries = new List<string>
        {
            path + "/GS1/AnimationDictionaries/characters.unity3d",
            path + "/GS1/AnimationDictionaries/sprites.unity3d",
            path + "/GS1/AnimationDictionaries/courts.unity3d",
            path + "/GS1/AnimationDictionaries/etcs.unity3d"
        };
        Gs1DictionaryCount = gs1Dictionaries.Count;
        List<string> gs2Dictionaries = new List<string>
        {
            path + "/GS2/AnimationDictionaries/characters.unity3d",
            path + "/GS2/AnimationDictionaries/sprites.unity3d",
            path + "/GS2/AnimationDictionaries/courts.unity3d",
            path + "/GS2/AnimationDictionaries/etcs.unity3d",
            path + "/GS2/AnimationDictionaries/psycolock.unity3d",
            path + "/GS2/AnimationDictionaries/ita.unity3d",
            path + "/GS2/AnimationDictionaries/itb.unity3d"
        };
        Gs2DictionaryCount = gs2Dictionaries.Count;
        List<string> gs3Dictionaries = new List<string>
        {
            path + "/GS3/AnimationDictionaries/characters.unity3d",
            path + "/GS3/AnimationDictionaries/sprites.unity3d",
            path + "/GS3/AnimationDictionaries/courts.unity3d",
            path + "/GS3/AnimationDictionaries/etcs.unity3d",
            path + "/GS3/AnimationDictionaries/psycolock.unity3d",
            path + "/GS3/AnimationDictionaries/ita.unity3d",
            path + "/GS3/AnimationDictionaries/itb.unity3d"
        };
    ...
    private IEnumerator LoadReplacementDictionaryFlow(string path)
    {
        List<string> loadTargets = new List<string>
        {
            path + "/GS1/AnimationDictionaries/foareplacementcharacters.unity3d",
            path + "/GS1/AnimationDictionaries/foareplacementobjects.unity3d",
            path + "/GS2/AnimationDictionaries/foareplacementcharacters.unity3d",
            path + "/GS2/AnimationDictionaries/foareplacementobjects.unity3d",
            path + "/GS3/AnimationDictionaries/foareplacementcharacters.unity3d",
            path + "/GS3/AnimationDictionaries/foareplacementobjects.unity3d"
        };
        foreach (string targetPath in loadTargets)
        {
            byte[] data = decryptionCtrl.instance.load(targetPath);
