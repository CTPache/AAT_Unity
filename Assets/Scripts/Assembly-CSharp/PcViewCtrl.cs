...
public class PcViewCtrl : MonoBehaviour
{
    ...
    public class FontImageData
    {
        ...
        private ushort[] GetFontCountArray(bool fallback = false)
        {
            string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
            switch (lang)
            {
                case "JAPAN":
                ...
                default:
                    return LangPackCtrl.GetFontCountArray(lang) ?? GetFontCountArray(true);
            }
        }

        private float GetFontFillAmount(bool fallback = false)
        {
            string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
            switch (lang)
            {
                ...
                case "CHINA_T":
                    return 0.0355f * (float)disp_count + 0.005f;
                default:
                    return LangPackCtrl.GetFontFillAmount(lang, font_type, disp_count) == 0 ? GetFontFillAmount(true) : LangPackCtrl.GetFontFillAmount(lang, font_type, disp_count - 1);// - 1;
            }
        }
...
public AssetBundleSprite bg_;
    ...
    public Vector3 Get_OBJ_OP4_008_DiffPosition(bool fallback = false)
    {
        string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
        switch (lang)
        {
            ...
            case "CHINA_T":
                return Vector3.right * 49f;
            default:
                return LangPackCtrl.Get_OBJ_OP4_008_DiffPosition(lang) == Vector3.zero ? Get_OBJ_OP4_008_DiffPosition(true) : LangPackCtrl.Get_OBJ_OP4_008_DiffPosition(lang);
        }
    }

    private Vector2 GetFacePhotoPosition(bool fallback = false)
    {
        string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
        switch (lang)
        {
            ...
            case "CHINA_T":
                return new Vector2(106f, -56f);
            default:
                return LangPackCtrl.GetFacePhotoPosition(lang) == new Vector2(0, 0) ? GetFacePhotoPosition(true) : LangPackCtrl.GetFacePhotoPosition(lang);
        }
    }

    private float GetFontSpritePosition_X(bool fallback = false)
    {
        string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
        switch (lang)
        {
            ...
            case "CHINA_T":
                return 133f;
            default:
                return LangPackCtrl.GetFontSpritePosition_X(lang) == 0 ? GetFontSpritePosition_X(true) : LangPackCtrl.GetFontSpritePosition_X(lang);

        }
    }

    private float[] GetFontSpritePosition_Y(int in_type, bool fallback = false)
    {
        string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
        if (in_type == 0)
        {
            switch (lang)
            {
                ...
                case "CHINA_T":
                    return new float[3] { -248f, -477f, -709f };
                default:
                    return LangPackCtrl.GetFontSpritePosition_Y(lang, in_type) ?? GetFontSpritePosition_Y(in_type, true);
            }
        }
        switch (lang)
        {
    ...
    private Vector3 GetCursorPosition(int in_type, int in_line, bool fallback = false)
    {
        string lang = fallback ? Language.langFallback[GSStatic.global_work_.language] : GSStatic.global_work_.language;
        switch (lang)
        {
            ...
            default:
                ...
                return LangPackCtrl.GetCursorPosition(lang) == 0 ? GetCursorPosition(in_type, in_line, true) : new Vector3(LangPackCtrl.GetCursorPosition(lang), GetFontSpritePosition_Y(in_type)[in_line]);
        }
    }
    ...
    public void PcViewInitialize(int type)
    {
        ...
        {
            num = 3;
            num2 = ((GSUtility.GetLanguageLayoutType(GSStatic.global_work_.language) != "JAPAN") ? 9 : 7);
        }
        ...
        string lang = GSStatic.global_work_.language;
        switch (lang)
        {
            case "JAPAN":
    ...
    public void PcViewNext()
    {
        ...
        if (font_sprite_[line].EndLine())
        {
            ...
            int num2 = ((GSUtility.GetLanguageLayoutType(GSStatic.global_work_.language) != "JAPAN") ? 7 : 5);
            if (line < num2)
            {
                string lang = GSStatic.global_work_.language;
                switch (lang)
