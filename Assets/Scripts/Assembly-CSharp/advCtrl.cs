...
public class advCtrl : MonoBehaviour
{
    ...
    private IEnumerator scenarioCoroutine(bool continue_data, bool is_init)
    {
        ...
        Debug.Log("scenarioCoroutine " + scenario_mdt);
    ...
    private void LoadTextData(ref ConvertTextData data, string path, string language)
    {
        string in_path;
        if (Language.languages.IndexOf(language) > 6)
        {
            in_path = Application.streamingAssetsPath + "/../LangPacks/" + language + "/" + path;
            if (!File.Exists(in_path))
            {
                Debug.Log("Falling back to " + in_path);
                in_path = Application.streamingAssetsPath + "/" + path;
            }
        }
        else
            in_path = Application.streamingAssetsPath + "/" + path;
